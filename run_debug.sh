...
function formatCommand() {
    ...
    echo -e "fomant target_path: $target_path" >> "$log_dir/debug.log"
    ...
    for ((i=0; i<${#remote_storage_process[@]}; i++)); do
        if [[ "${remote_storage_process[i]}" == "${remote_storage_init[0]}" ]]; then
            rclone_command[i]="rclone copyto -v --config=/path/rclone/rclone.conf --include \"{$filename}.*\" \"${in_path}\" \"NASTOOL:${remote_dest}${target_path/'/TV/Anime'/${fields[1]}}\" --log-file=\"/tmp/${fields[2]}.${remote_storage_process[i]}\" --ignore-existing"
        else
            rclone_command[i]="rclone copyto -v --config=/path/rclone/rclone.conf --include \"{$filename}.*\" \"${in_path}\" \"backup:${target_path/'/TV/Anime'/${fields[1]}}\" --log-file=\"/tmp/${fields[2]}.${remote_storage_process[i]}\" --ignore-existing"
        fi
    done
}
...
function rcloneUpload() {
    ...
    for ((i=0; i<${#rclone_command[@]}; i++)); do
        {
            echo -e "rclonecommand[$i]: ${rclone_command[i]}" >> "$log_dir/debug.log"
...
function fetchAlistRefreshPathApi() {
    ...
    echo -e "alist_path=${target_path/'/TV/Anime'/${fields[1]}}" >> "$log_dir/debug.log"
...
function rcloneVfsRefresh() {
    local target_path="${in_path//$nastool_map_path/}"
    echo -e "rcloen_path=${target_path/'/TV/Anime'/${fields[1]}}" >> "$log_dir/debug.log"
...
function fetchEmbyRefreshMediaApi() {
    ...
    echo -e "${target_path/'/TV/Anime'/${fields[1]}}" >> "$log_dir/debug.log"
...
function log() {
    echo -e "status_code: $status_code" >> "$log_dir/debug.log"
